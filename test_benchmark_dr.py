...
from controller_only_route_vlm import RouteOnlyOvisController
...
def main():
    ...
    vision_module = Ovis3DPerceptionModule(model_path=MODEL_PATH)
    controller = RouteOnlyOvisController(model=vision_module.model)
    depth_module = DepthPro3DModule()
    knowledge_module = GeometryKnowledgeInjector(
        kb_dir="/root/autodl-tmp/knowledge_bases", 
        use_common_dimensions=USE_COMMON_DIMENSIONS
    ...
    for item in tqdm(test_subset):
        ...
        try:
            ...
            controller_output = controller.predict(item["Question"], options_text)
            task_route = controller_output["route"]
            knowledge_tags = []   # 先留空
            controller_reason = controller_output["reason"]
            ...
            if task_route == "line0_pure_vlm":
                ...
                # external_knowledge_text = knowledge_module.inject_knowledge_by_tags(
                #     [],
                #     knowledge_tags,
                #     item["Question"]
                # )
                ...
                scene_graphs_dict[task_id] = {
                    ...
                    "predicted_knowledge_tags": knowledge_tags,
                    "objects": [],
                    "spatial_relations_text": f"Bypassed via Pure VLM.\n{external_knowledge_text}"
                }
